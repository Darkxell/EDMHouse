...
public class MpegAudioFileReader extends TAudioFileReader {
    ...
    private static final String[] id3v1genres = { "Blues", "Classic Rock",
	...
	pis.read(head);
	if (TDebug.TraceAudioFileReader) {
	    TDebug.out("InputStream : " + inputStream + " =>"
		    + new String(head));
	}
	...
	try {
	    ...
	    if ((streamPos > 0) && (mLength != AudioSystem.NOT_SPECIFIED)
		    && (streamPos < mLength))
		...
		aff_properties.put("duration",
			new Long((long) nTotalMS * 1000L));
	    }
	    aff_properties.put("mp3.copyright",
		    new Boolean(m_header.copyright()));
	    aff_properties
		    .put("mp3.original", new Boolean(m_header.original()));
	...
	if ((size == mediaLength) && (mediaLength != AudioSystem.NOT_SPECIFIED)) {
	    ...
	    long bytesSkipped = fis
		    .skip(inputStream.available() - id3v1.length);
	...
	AudioFormat format = new MpegAudioFormat(encoding, (float) nFrequency,
		AudioSystem.NOT_SPECIFIED // SampleSizeInBits - The size of a
					  // sample
		, nChannels // Channels - The number of channels
		, -1 // The number of bytes in each frame
		, FrameRate // FrameRate - The number of frames played or
			    // recorded per second
		, true, af_properties);
	...
	try {
	    if (TDebug.TraceAudioFileReader)
		TDebug.out("ID3v2 frame dump='"
			+ new String(bframes, 0, bframes.length) + "'");
	    /*
	     * ID3 tags :
		...
		if (v2version == 3 || v2version == 4) {
		    ...
		    i += 10;
		    if ((code.equals("TALB")) || (code.equals("TIT2"))
			    || (code.equals("TYER")) || (code.equals("TPE1"))
			    || (code.equals("TCOP")) || (code.equals("COMM"))
			    || (code.equals("TCON")) || (code.equals("TRCK"))
			    || (code.equals("TPOS")) || (code.equals("TDRC"))
			    || (code.equals("TCOM")) || (code.equals("TIT1"))
			    || (code.equals("TENC")) || (code.equals("TPUB"))
			    || (code.equals("TPE2")) || (code.equals("TLEN"))) {
			if (code.equals("COMM"))
		    ...
		    i += 6;
		    if ((scode.equals("TAL")) || (scode.equals("TT2"))
			    || (scode.equals("TP1")) || (scode.equals("TYE"))
			    || (scode.equals("TRK")) || (scode.equals("TPA"))
			    || (scode.equals("TCR")) || (scode.equals("TCO"))
			    || (scode.equals("TCM")) || (scode.equals("COM"))
			    || (scode.equals("TT1")) || (scode.equals("TEN"))
			    || (scode.equals("TPB")) || (scode.equals("TP2"))
			    || (scode.equals("TLE"))) {
			if (scode.equals("COM"))
			    value = parseText(bframes, i, size, 5);
			else
	...
	try {
	    ...
	    value = new String(bframes, offset + skip, size - skip,
		    ENC_TYPES[bframes[offset]]);
